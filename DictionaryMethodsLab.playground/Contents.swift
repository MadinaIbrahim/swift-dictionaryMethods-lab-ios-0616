...
for (key, _ ) in starWarsCharacters {
    print("\(key)") // All four keys are printed.
}

// or: 

let keysForStarWarCharacters = Array(starWarsCharacters.keys)
for keys in keysForStarWarCharacters {
    print("\(keys)")
}
 ...
 */
let numberOfKeys = starWarsCharacters.count
    print("\(numberOfKeys)")  // 4 is printed to the console
/*: question11
...
if starWarsCharacters.isEmpty {
    ...
    print("There are \(numberOfKeys) items in the dictionary.")
}
// Sins dictionary is not empty, we got else statement printout.
/*: question12
 ...
 */
let newDictionary = starWarsCharacters.updateValue(starWarsJedi, forKey: "Jedi")
 ...
 */
let anotherNewDictionary = starWarsCharacters.removeValueForKey("Bounty Hunters")
if anotherNewDictionary == nil {
...
if let jediCharacters = starWarsCharacters["Jedi"] {
    print("\(jediCharacters)")
}
/*: question16
